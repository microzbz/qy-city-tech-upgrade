...
public class MsgCenterService {
    ...
    public void sendApprovalResult(Long submissionId, SubmissionStatus status, String actionName, String comment) {
        AppProperties.External.MsgCenter config = appProperties.getExternal().getMsgCenter();
        if (!config.isEnabled()) {
        ...
        try {
            sendApprovalResultInternal(submissionId, status, actionName, comment, config);
        } catch (Exception e) {
    ...
    private void sendApprovalResultInternal(Long submissionId,
        ...
        if (form == null) {
            log.warn("[通知中心] 提交通知时未找到单据，submissionId={}", submissionId);
            return;
        }
        EnterpriseProfile enterprise = enterpriseProfileRepository.findById(form.getEnterpriseId()).orElse(null);
        if (enterprise == null) {
            log.warn("[通知中心] 提交通知时未找到企业，submissionId={}，enterpriseId={}", submissionId, form.getEnterpriseId());
            return;
        }
        ...
        String receiveParam = resolveReceiveParam(receiveParamType, enterprise);
        if (!StringUtils.hasText(receiveParam)) {
            log.warn("[通知中心] 联系人接收参数缺失，submissionId={}，enterpriseId={}，receiveParamType={}，contactName={}，contactCertNo={}，contactPhone={}",
                submissionId, enterprise.getId(), receiveParamType, enterprise.getContactName(), enterprise.getContactCertNo(), enterprise.getContactPhone());
            return;
        }

        String templateId = require(config.getTemplateId(), "app.external.msg-center.template-id 未配置");
        Map<String, Object> dataJson = buildDataJson(submissionId, form, basic, enterprise, status, actionName, comment, receiveParam);
        Map<String, Object> extData = buildExtData(submissionId, form, basic, enterprise, status, actionName, comment, receiveParamType, receiveParam);
        ...
        log.info("[通知中心] 开始发送审批结果，submissionId={}，documentNo={}，enterpriseId={}，templateId={}，receiveParamType={}，receiveParam={}，dataJson={}，extData={}",
            submissionId, resolveDocumentNo(form, basic, submissionId), enterprise.getId(), templateId, receiveParamType, receiveParam,
            params.get("dataJson"), params.get("extData"));
        ...
        Map<String, Object> sendResponse = callAccess(baseUrl, token, require(config.getSendApiName(), "app.external.msg-center.send-api-name 未配置"), params, policyPwd);
        ...
        log.info("[通知中心] 审批结果发送完成，submissionId={}，documentNo={}，messageId={}，receiveParamType={}，receiveParam={}，actionName={}",
            submissionId, resolveDocumentNo(form, basic, submissionId), messageId, receiveParamType, receiveParam, actionName);
        ...
        if (config.isQueryResultEnabled() && StringUtils.hasText(messageId)) {
            ...
            log.info("[通知中心] 审批结果发送状态查询完成，submissionId={}，messageId={}，result={}", submissionId, messageId, resultResponse);
        }
    }

    private Map<String, Object> buildDataJson(Long submissionId,
                                              SubmissionForm form,
    ...
    private Map<String, Object> buildExtData(Long submissionId,
                                             SubmissionForm form,
                                             SubmissionBasicInfo basic,
                                             EnterpriseProfile enterprise,
                                             SubmissionStatus status,
        ...
        data.put("enterpriseName", resolveEnterpriseName(basic, enterprise));
        data.put("documentNo", resolveDocumentNo(form, basic, submissionId));
        ...
        data.put("contactName", enterprise.getContactName());
        data.put("contactCertNo", enterprise.getContactCertNo());
        data.put("contactCertType", enterprise.getContactCertType());
